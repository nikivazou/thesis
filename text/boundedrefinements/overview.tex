...
\paragraph{Refinement Types} let us precisely specify subsets of values,
...
and so on. Apart from subsets of values, like the @Pos@ and @IntGE@
that we saw in the introduction, we can specify contracts
like pre- and post-conditions by suitably refining the input
and output types of functions.

\paragraph{Preconditions} are specified by refining input types.
We specify that the function @assert@ must
\emph{only} be called with @True@,
%
where the refinement type @TRUE@ contains only the singleton @True@:
%
\begin{code}
...
%%%%
%%%\begin{mcode}
%%%  a:Int, b:IntGE a, cmp:{Bool | cmp <=> a <= b}
%%%    ||- {v : Bool | v = cmp} <: TRUE
%%%\end{mcode}
%%%%
%%%Intuitively, the relation @G ||- T <:  U@ may be understood as
%%%%
%%%``assuming the type bindings in @G@, @T@ is a subtype of @U@''~\cite{vazou13}.
%%%%
%%%Subtyping between refinement types reduces to checking the validity of
%%%the \emph{logical implication} between the sub- and supertype's
%%%refinements in the current environment.
%%%%
%%%The above subtyping check reduces to the following
...
\paragraph{Abstract Refinements} overcome the above modularity
...
\paragraph{At the call to \cc{find}} in the body of @ex1@, we perform
...
@checkGE@ cannot fail.
%
%%% \ab{This is confusing: based on the previous text it doesn't sound
%%%   like we need to synthesize anything. The constraint was completely
%%%   determined by our choice of p and the definition of UpClosed.}
%%% Crucially, we can also \emph{automatically synthesize} instantiations
%%% that satisfy the constraint via the framework of Liquid Typing
%%% \cite{LiquidPLDI08}, which makes using such bounds palatable in
%%% practice.
...
\paragraph{\cc{filter} can be given a type} saying that the output values
...
\paragraph{We specify induction as a bound.} Let
...
\paragraph{We can give \cc{foldr} a type} that says that the
...
%% \nv{addition:}
Previously, \cite{vazou13} describes a way to type @foldr@
...
%
We solve both problems via a unifying insight inspired
...
of \emph{unbounded} abstract refinement typing~\cite{vazou13}.
%
...
\paragraph{We Translate Bounds into Function Types} called the
...
\paragraph{To Check Bounded Functions} we view the bound constraints
...
\paragraph{At Uses of Bounded Functions} our elaboration uses
...
\inter{\Gamma}
  ...
  \Rightarrow \cc{(z == y + 1)}
  & \Rightarrow \cc{(y == x + 1)}\notag\\
  & \Rightarrow \cc{(z == x + 2)} \label{vc:ex2}
\end{align}
%
%
...
scope.
%
The VC \ref{vc:ex2} is easily proved valid by an SMT solver, thereby
