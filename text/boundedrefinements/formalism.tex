...
\paragraph{The Primitive Constants} of \corelan include
...
\paragraph{The Types} of \boundedcorelan extend those of \corelan with
...
%
%%% We get the non-refined variant of a type $\rtyp$ by the
%%% function $\totrue{\rtyp}$, defined:
%%% %
%%% \begin{align*}
%%% \totrue{\tref{b}{\reft}} \defeq & \tref{b}{\true} \\
%%% \totrue{\trfun{x}{\rtyp_1}{\rtyp_2}{\reft}} \defeq & \trfun{x}{\totrue{\rtyp_1}}{\totrue{\rtyp_2}}{\true}
%%% \end{align*}
%%% %
...
%%Axiomatic functions are function that are used to represent local axioms,
%%or constraints of functions. Calling these functions is equivalent to instantiating
%%the axiom; while the definition of these axioms is equivalent to proving the axiom.

%% With that we define the translation of expressions as
%% \begin{definition}
%% $ \tx{e} = e' \Leftrightarrow \txexpr{\emptyset}{\emptyset}{e}{e'}$.
%% \end{definition}
...
%%% which is a function type that
%%% The type $\txbound{\constraint}$ is a function type that
%%% has arguments the elements of the environment and
%%% the type over which the constraint applies
%%% and has result a $\tbunit$ type whose refinement
%%% 1)~ depends only on the inputs and 2)~ guarantees the constraint $\phi$.
%%% The translation of a constraint to a function type in formally defined as follows
%%% %%% \txbound{\isSubType{x_1\colon r_1, \dots, x_n\colon r_n}
%%% 	{\tpref{b}{p_1}{\reft_1}}
%%% 	{\tpref{b}{p_2}{\reft_2}}} \defeq \\
%%% {x_1}\colon{\toshape{r_1}} \rightarrow \dots \rightarrow
%%% {x_n}\colon{\toshape{r_n}} \rightarrow
%%% x\colon b \rightarrow
%%% \tref{\tbunit}{  q \Rightarrow q_1 \Rightarrow q_2}
%%% \end{align*}
%%% with
%%% $ q = \bigwedge_{i = 1 \dots n} \tologic{x_i}{r_i}$,
%%% $ q_i = \tologic{x}{\tpref{b}{p_i}{\reft_i}}$
%%%
%%% and
%%% $\tologic{x}{\tpref{b}{\areft}{\reft}} = \reft\sub{\vref}{x} \land \areft\ x$

%%% Having the axiomatic function $f \colon \txbound{\phi}$
%%% at scope is very powerful because application of that
%%% function, \ie $f\ e_1 \dots e_n\ e$, strengthens the
%%% type of the last argument $e$
%%% from $e : \tpref{b}{p_1}{\reft_1}$ to $e : \tpref{b}{p_2}{\reft_2}\sub{x_i}{e_i}$.
...
%%%  & \defeq & \{ \ e  \ | \ \tbind{f}{\utyp} \leftarrow \benv \\
%%%  &        & \quad \ \ , \ \overline{\tbind{x}{\_}} \leftarrow \cenv \\
%%%  &        & \quad \ \ , \ e = f\ \overline{x}  \\
%%%  &        & \quad \ \ , \ \EXT{\cenv}{f}{\utyp} \vdash_B \tbind{e}{\tbbool} \} \
\end{array}$$
...
\paragraph{3. Rule~\txCApp} translates bound applications
...
\paragraph{The Translation is Semantics Preserving} in the sense that
if a source term $e$ of $\boundedcorelan$ reduces to a constant
then the translated variant of $e'$ also reduces to the same
constant:% \ifextended{:}{(as show in~\citep{techrep}):}

\begin{lemma*}\ifextended{[Semantics Preservation]}{}
\label{theorem:operational}
...
\paragraph{The Soundness of \boundedcorelan} follows by combining
...
\paragraph{We restrict implications to bounds} \ie prohibit
...
%%%%% Ultimately, this constraint means that abstract refinement variables
%%%%% only appear in positive positions in the right hand side $\reft_2$
%%%%% of sub-typing constraints
%%%%% %
%%%%% $
%%%%% \isSubType{\Gamma}{\stref{b}{\reft_1}}{\stref{b}{\reft_2}}
%%%%% $.
%%%%% %
%%%%% Our syntax does not, in general, enforce this restriction as it allows
%%%%% atomic refinements, and thus $\rvar$ variables, to appear in
%%%%% negative positions, which is crucial for expressiveness.
%%%%% %
%%%%% Thus, we can get sub-typing constraints of the form
...
%%%
%%%\rj{TODO}
%%%%%Colins notes on Inference:
%%%%%It may be worth noting that these refinement bounds actually make type inference work *better* in some
%%%%%instances.  I got to remove an ‘undefined’ witness term from my lock free data structures with their addition.
%%%%%Not sure if/where this fits in the paper’s story though.
%%%
%%% OLD $$
%%% OLD \closure{\cenv}{\benv}{e}{\tbind{x}{\utyp}} \defeq
%%% OLD \left\{
%%% OLD 	\begin{array}{ll}
%%% OLD 	\elet{x_1}{e_1}{}{\tbbool}&\\
%%% OLD 	 \dots &  \text{if} \ \tau = r\\
%%% OLD 	 \elet{x_n}{e_n}{e}{\tbbool}&\\
%%% OLD 		& \\
%%% OLD 		e  & \mbox{otherwise}\\
%%% OLD 	\end{array}
%%% OLD \right.
%%% OLD $$
%%% OLD
%%% OLD where  fresh $x_i$ and
%%% OLD $$
%%% OLD \begin{array}{rl}
%%% OLD \{ e_1, \dots , e_n\} \defeq \{e \mid & f\colon t_f \in \Phi, e = f\ x_1 \dots x_n, \\
%%% OLD & x_i \in \Gamma \lor x_i = x, \exists i. x_i = x  \\
%%% OLD &\Delta, f\colon t_f, x\colon \toshape{\tau} \vdash_B e \colon \tbunit \}
%%% OLD \end{array}
%%% OLD $$
...
%%% \inference{
%%% 	\text{fresh}\ x &&
%%% 	\txexpr{\cenv}{\Phi, x\colon\toshape{\tx{\phi}}}{e}{e'}
%%% }{
%%% 	\txexpr{\cenv}{\Phi}{}{\efunt{x}{\tx{\phi}}{e'}}
%%% }[\txCAbs]
%%% $$
%%%
%%% $$
...
%%% 	\txexpr{\cenv}{\Phi}{e}{e'}
%%% }{
%%% 	\txexpr{\cenv}{\Phi}{\econstantconstraint{e}{\phi}}{\eapp{e'}{\ctofun{\phi}}}
%%% }[\txCApp]
%%% $$
