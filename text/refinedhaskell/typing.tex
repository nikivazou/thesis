...
%%% \NV{Can we combine Fig 5 and 6?  Otherwise r::=p in Fig 5 looks really odd.
%%% Fig 5: since predicates include terms, is ther any merit in having both p and u?  Then can't =, > be ordinray infix ops?
%%% [12:58:21 PM] Simon Peyton Jones: Fig 5, can we say:
%%% Constants ::= ...as Fig 3 ...
%%% Values ::= as fig 3
%%% }
%%% \NV{Fig 6:p,u ::= ....}
%%% \NV{Remove D from u, }
%%% \NV{We use p for boolean-valued terms, which we also call predicates
%%% }
...
%% \NV{DONE: remove Measures \& Axioms}
%%\mypara{Measures \& Axioms} 
%%The only function symbols in \logiclang correspond to 
%%\emph{measures} (as described in \Sref{sec:measures}), 
%%which are inductively defined functions.
%%%%Each measure definition is a sequence of equations,
%%%%one per data constructor, where the body of each 
%%%%equation is a term in \logiclang.
%%%
%%In the metatheory, we can characterize measures using axioms translated from the measure definition.
%%%
%%For example, @emp@ from \Sref{sec:overview} yields axioms:
%%%
%%\begin{align*}
%%%%  \forall x.      & x = \dnull        \Rightarrow (\leisNull\ x) \\
%%%%  \forall x,y,ys. & x = \dcons\ y\ ys \Rightarrow \lnot (\leisNull\ x) 
%%  (\ttemp\ \dnull) &&
%%  \forall x,xs. \lnot (\ttemp\ (\dcons\ x\ xs))
%%\end{align*}
%%\SPJ{What on earth does this (above) mean?}
...
%% \NV{This is not enough, as variables are still diverging, 
%% we need termination analysis}
...
theoretical and practical problems, as discussed in \Sref{sec:conclusion}, 
...
\end{itemize} %%NV each -> all to fit in 1 line after b -> B
